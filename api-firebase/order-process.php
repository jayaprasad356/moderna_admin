...
if (isset($_POST['update_order_items']) && $_POST['update_order_items'] == 1) {
    ...
    if (!empty($order_items)) {
        for ($j = 0; $j < count($order_items); $j++) {
            ...
            // $res_order_item = $function->get_data($columns = ['user_id', 'order_id', 'active_status', 'status'], 'id=' . $order_item_id, 'order_items');
...
if (isset($_POST['add_pickup_location']) && $_POST['add_pickup_location'] == 1) {
    ...
    // echo $token;
    // return false;
    $pickup_location = $shiprocket->add_pickup_location($pickup_location_data, $token);
    $pickup_location = json_decode($pickup_location, 1);
    ...
    */
    ...
    if (isset($_FILES['image']) && ($_FILES['image']['size'] > 0)) {
        ...
        for ($i = 0; $i < count($_FILES["image"]["name"]); $i++) {
            ...
            $file_data[] = $filename;
            if (!move_uploaded_file($_FILES["image"]["tmp_name"], '../upload/attachments/' . '' . $filename)) {
                $response['error'] = true;
                $response['message'] = "Images not uploaded!";
                print_r(json_encode($response));
                return false;
    ...
    foreach ($res as $row) {
        ...
        foreach ($res_attac as $item) {
            array_push($myData, ['id' => $item['id'], 'image' => DOMAIN_URL . 'upload/attachments/' . $item['attachment']]);
        }
        ...
        $res[$i]['final_total'] = $res[$i]['final_total'];
        $res[$i]['date_added'] = date('d-m-Y h:i:sa', strtotime($res[$i]['date_added']));
        $res[$i]['status'] = json_decode($res[$i]['status']);
        ...
        $sql = "select oi.*,p.id as product_id,v.id as variant_id,p.name,p.image,p.manufacturer,p.made_in,p.return_status,p.cancelable_status,p.till_status,v.measurement,(select short_code from unit u where u.id=v.measurement_unit_id) as unit from order_items oi left join product_variant v on oi.product_variant_id=v.id left join products p on p.id=v.product_id where order_id=" . $row['id'] . " GROUP BY oi.id";
        $db->sql($sql);
        $res[$i]['items'] = $db->getResult();
        for ($j = 0; $j < count($res[$i]['items']); $j++) {
            $res[$i]['items'][$j]['status'] = (!empty($res[$i]['items'][$j]['status'])) ? json_decode($res[$i]['items'][$j]['status']) : array();
